...
public class FlowDensitySi extends AppCompatActivity {
    ...
    public ArrayList<String> DongOfGu(String gu){
        ...
        for(i=0;i <SeoulAreaList.size() ; i++) {
            ...
            if(SeoulAreaList.get(i).get("siGunGu").equals(si)){
                list.add(SeoulAreaList.get(i).get("lDongCd").toString());
            }
        }
        return list;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i<DongOfGu(si).size(); i++) {
            safeCasterApi.getFlowDensity_dong(key,
                    format_time2, getDong(i)).enqueue(new Callback<FlowDensity_dong>() {
                @Override
    ...
    public String getDong(int i){
        String dong;
        dong = DongOfGu(si).get(i);
        return dong;
    }
    ...
    private void init() {
        ...
        adapter = new RecyclerAdapter_si(getData(),FlowDensitySi.this, si, DongOfGu(si));
    ...
    private ArrayList getData() {
        ...
        for (int i = 0; i<DongOfGu(si).size(); i++){
            if(FindDongByCd(DongOfGu(si).get(i))==null)
                continue;
            Data data = new Data();
            data.setTitle(FindDongByCd(DongOfGu(si).get(i)));
            itemList.add(data);
            data.setGuName(si);
        }
